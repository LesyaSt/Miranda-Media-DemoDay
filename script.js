document.addEventListener("DOMContentLoaded", function () {
    ...
    let currentFilter = "Novinky"; // Фільтр за замовчуванням тепер відповідає JSON
    let allProducts = [];
    ...
    function filterAndRenderProducts(filter) {
        ...
        // Перетворюємо назву категорії у правильний формат
        const filterMap = {
            novinky: "Novinky",
        ...
        const filteredProducts = allProducts.filter((product) => {
            return product.category === category; // Порівнюємо з точною назвою
        });

        renderProducts(filteredProducts.slice(0, displayedProducts));

        loadMoreBtn.style.display = (filteredProducts.length > displayedProducts) ? "block" : "none";
    }
    ...
    function renderProducts(products) {
        products.forEach((product) => {
            ...
            if (product.availability === "Momentálně nedostupné") statusClass = "span-unavailable";

            const labels = product.flags.length
                ? product.flags.map(flag => {
                    if (flag === "Novinka") return `<span class="label-new">${flag}</span>`;
                    if (flag === "Výprodej") return `<span class="label-sale">${flag}</span>`;
